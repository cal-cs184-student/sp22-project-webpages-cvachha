...
![Part 1](images/cs184_proj2_bezier_1.png)

(.bzc file is `bzc/curve3.bzx`)
...
![Part 1](images/cs184_proj2_bezier_2.png)

![Part 1](images/cs184_proj2_bezier_3.png)
...
We can pre-process the cube to allow for symmetrical subdivisions by splitting the diagonal edge across each face. After subdividing multiple levels, the mesh appears symmetrical. This is because these subdivisions make the cube more symmetric to begin with and that we are defining more parts of the original cube mesh with these edges, so when it gets subdivided, it will retain more of the original cube shape and structure. Pre-processing helps maintain the original structure of the mesh so subdividing does not significantly change the overall shape of the mesh.
...
![Part 7](images/lego_blender_screenshot.png)
...
The final 3D models are: `partsevenmodel.dae` and `partsevenmodel_unity.dae` located in the `dae` folder and `docs` folder. 


![Part 7](images/cs184_proj2_7_2.png)

When subdividing the model, I made sure to add bevels and edge loops to make sure that the original structure of the model remains.
